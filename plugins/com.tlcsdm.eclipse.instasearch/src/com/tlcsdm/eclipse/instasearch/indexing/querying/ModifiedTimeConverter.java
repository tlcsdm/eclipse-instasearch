...
public class ModifiedTimeConverter extends QueryVisitor {
	...
	private enum Interval {
		TODAY, YESTERDAY, HOUR(TimeUnit.HOURS.toMillis(1)), DAY(TimeUnit.DAYS.toMillis(1)),
		WEEK(TimeUnit.DAYS.toMillis(7)), MONTH(TimeUnit.DAYS.toMillis(30));
	...
	public Query visit(TermQuery termQuery, Field termField) {
		...
		// In Lucene 9.x, TermRangeQuery constructor takes BytesRef
		return new TermRangeQuery(field, new BytesRef("" + start), new BytesRef("" + end), true, true);
	}
