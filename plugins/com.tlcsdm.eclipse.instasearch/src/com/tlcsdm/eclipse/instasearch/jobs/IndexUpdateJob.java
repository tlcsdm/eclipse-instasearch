 ...
 */
package com.tlcsdm.eclipse.instasearch.jobs;
 ...
 */
public class IndexUpdateJob extends WorkspaceJob
		implements SynchronousBundleListener, IResourceChangeListener, IPropertyChangeListener {

	private WorkspaceIndexer indexer;
	private IndexChangeListener indexChangeListener;

	private Map<IResource, Integer> changedResources = new ConcurrentHashMap<IResource, Integer>(); // changed by
																									// several threads
	...
	public IndexUpdateJob(WorkspaceIndexer indexer, IndexChangeListener indexChangeListener) {
		...
		ResourcesPlugin.getWorkspace().addResourceChangeListener(this, IResourceChangeEvent.POST_CHANGE); // listen to
																											// file
																											// changes

		if (InstaSearchPlugin.getDefault() != null)
			InstaSearchPlugin.getDefault().getBundle().getBundleContext().addBundleListener(this); // listen to plugin
																									// stopping
	...
	public void resourceChanged(IResourceChangeEvent event) {
		if (event.getType() == IResourceChangeEvent.PRE_BUILD || event.getType() == IResourceChangeEvent.POST_BUILD)
		...
		try {
			if (indexer.isIndexed()) {
				...
				// else if( event.getResource().getType() == IResource.PROJECT ) //
				// CLOSE,DELETE,REFRESH events for project
	...
	public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
		...
		try {
			for (Iterator<IResource> iterator = changedResources.keySet().iterator(); iterator.hasNext()
					&& !monitor.isCanceled();) {
			...
			PlatformUI.getWorkbench().getDisplay().syncExec(new Runnable() {
				public void run() {
					searchViewVisible = InstaSearchUI.isSearchViewVisible(); // must run in UI thread, but this job
																				// isn't
				}
			});
