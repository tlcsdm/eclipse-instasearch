...
/**
 * A search job that runs search in UI thread. Is used to start search after
...
public class SearchJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		if (prov == null || monitor.isCanceled() || searchQuery == null || searchQuery.isCanceled())
			return Status.CANCEL_STATUS;

		prov.getElements(searchQuery); // runs query and caches result. running in this job to avoid UI freeze

		if (monitor.isCanceled() || searchQuery.isCanceled())
			return Status.CANCEL_STATUS; // perhaps we cancelled while executing query
	...
	private void search() {
		...
		if (searchQuery.isLimited()) // limited
		{
			if (resultCount >= searchQuery.getMaxResults())
				statusLineManager.setMessage(searchView.getTitleImage(),
						searchQuery.getMaxResults() + "+" + resultsDesc + " (Ctrl+Enter to see all)");
			else
