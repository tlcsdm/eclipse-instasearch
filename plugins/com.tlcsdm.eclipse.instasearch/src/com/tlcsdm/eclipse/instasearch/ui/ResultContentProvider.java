...
import com.tlcsdm.eclipse.instasearch.InstaSearch;
...
import com.tlcsdm.eclipse.instasearch.indexing.SearchResult;
import com.tlcsdm.eclipse.instasearch.indexing.SearchResultDoc;
import com.tlcsdm.eclipse.instasearch.indexing.Searcher;
...
class ResultContentProvider implements ITreeContentProvider {
	...
	public static final int MAX_LINES_TO_PROCESS = 5000; // max nr of lines to read from files (prevent slowdown on huge
															// files)
	...
	public Object[] getElements(Object searchQueryObj) {
		...
		if (searchQuery.isLimited() && result.isFull()) { // if only showing limited number of matches
			...
			if (searchQuery.isExact() && !searchQuery.isFuzzy()) // if query is exact, can try search for individual
																	// tokens
				addFindSimilar = true;
		}
	...
	/**
	 * Returns last search result elements. Includes search result docs and
	 * additional action entries (eg More results)
	 * 
	 ...
	 * @return
	 */
	private float addMatches(MatchLine matchLine, Map<String, List<Integer>> terms, Set<String> matchedTerms,
			String searchString) {

		String lcaseLine = matchLine.getLine().toLowerCase(Locale.ENGLISH);

		if (!matchedTerms.contains(searchString) && !currentSearchQuery.isFuzzy()) { // check for exact match on the
																						// line
	...
	/**
	 * Removes similar lines from line matches. Even if they are high scored, we don
	 * not want to see the same lines again Line similarity is based on Cosine
	...
	private void removeSimilarLines(List<MatchLine> matchedLines, int maxMatches) {
		...
		int lineNr = 0;
		for (Iterator<MatchLine> iterator = matchedLines.iterator(); iterator.hasNext()
				&& matchedLines.size() > maxMatches;) {
	...
	/**
	 * Calculates similarity based on the Cosine angle between score vectors of each
	 * line.
	 * 
	 * @param lineMatches1
	 * @param lineMatches2
	...
	class MatchLine implements Comparable<MatchLine> {
		...
		private MatchLine(SearchResultDoc doc, String lineText, int lineNumber, Set<String> matchedTerms,
				float[] termScoreVector, float[] matchedTermScoreVector) throws IOException {
			this.doc = doc;
